...
def vignetting_limits(slit,nexp,wave):
    ...
    # A SLIT NOT OBSERVED IN THE REQUESTED EXPOSURE (E.G. A SERENDIP
    # ABSENT FROM THE FIRST EXPOSURE OF A MULTI-EXPOSURE MASK) COMES BACK
    # WITH A MASKED `det` VALUE. np.ma.masked PROPAGATES THROUGH == AND &
    # AS A MASKED RESULT RATHER THAN A PLAIN bool, WHICH RAISES TypeError
    # BELOW INSTEAD OF JUST BEING FALSY (FOUND 2026-08-27, leoa/SERENDIP).
    # NO DETECTOR INFO FOR THIS EXPOSURE MEANS NO VIGNETTING CORRECTION
    # CAN BE DETERMINED -- FALL BACK TO THE DEFAULT FULL WAVELENGTH RANGE.
    if np.ma.is_masked(det):
        return (wave > vwave_min) & (wave < vwave_max)
