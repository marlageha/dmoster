...
def fit_map_anchor_with_sigma(wvl, spec, ivar, mw23, center_margin=1.0):
    '''Same MAP fit as fit_anchor_stage1 (EW2+EW3 only), but also
    returns the local inverse-Hessian-derived uncertainty on p2/p3 --
    used to set the Stage 1 soft-anchor prior width below. Kept
    separate from fit_anchor_stage1 itself so fit_decoupled_stage_
    missing (the missing-coverage path, which only needs the point
