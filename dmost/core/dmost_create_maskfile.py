...
def create_slits(nslits,nexp):
    ...
    cols = [filled_column('objname','                ',nslits),
           ...
           # EQUIVALENT WIDTHS FROM COADD1D FILES
            ...
            filled_column('cat_theta',-999.*np.ones(14),nslits),   # POSTERIOR MEDIAN OF THE ADAPTIVE GL-gvary FIT -- LETS QA REBUILD THE EXACT PRODUCTION FIT CURVE WITHOUT RE-FITTING. 14 SLOTS: STAGE 0/A USE 13 (DECOUPLED-WIDTH MODEL), STAGE B USES ALL 14 (FREE CENTERS ADDED)
            filled_column('cat_chi2',-999.,nslits),
            filled_column('cat_f_acc',-999.,nslits),
            filled_column('cat_converge',-999.,nslits),
